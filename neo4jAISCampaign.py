...
class App:
    ...
    def query_shipping_sites(self,startID,endID):
        ...
        with self.driver.session(database=currDB) as session:
            ...
            query = (
            "match (c:Campaign) with c skip $startID limit $endID call { with c match (u:URL) where u.adv_entity_id = c.adv_entity_id MERGE (u)<-[r:HAS_ADS]-(c) ON CREATE SET r.ctr=u.clicks, r.volume = u.counter } IN TRANSACTIONS OF 1 rows;"
            )   
